...
[一、项目概述 4](#_Toc231653218)

[1.1 定位 4](#_Toc231653219)

[1.2 痛点分析 4](#_Toc231653220)

[1.3 创新性与价值 5](#_Toc231653221)

[二、技术路线与核心选型 5](#_Toc231653222)

[2.1 核心技术要素 5](#_Toc231653223)

[2.2 技术栈选型 6](#_Toc231653224)

[三、系统架构设计 7](#_Toc231653225)

[3.1 整体架构 7](#_Toc231653226)

[3.2 LangGraph 状态机 8](#_Toc231653227)

[四、多智能体协作设计 9](#_Toc231653228)

[4.1 Preference Agent 9](#_Toc231653229)

[4.2 Mediator Agent 9](#_Toc231653230)

[五、记忆与工具设计 10](#_Toc231653231)

[5.1 向量数据库记忆机制 10](#_Toc231653232)

[5.2 MCP / Function Calling 设计 10](#_Toc231653233)

[六、MVP 开发与里程碑 11](#_Toc231653234)

[6.1 MVP 简化路径 11](#_Toc231653235)

[6.2 开发时间表 11](#_Toc231653236)

[七、GitHub 仓库规划 12](#_Toc231653237)
...
- collect_prefs：收集所有 Preference Agent 的观影偏好
- detect_conflict：检测偏好冲突（类型互斥、时长超限等）
- resolve：冲突不可调和时，Mediator Agent 启动妥协规则
- generate_proposal：基于协调后的偏好生成候选影片提案
- collect_votes：各 Preference Agent 对提案投票
- final_decision：达成群体共识，输出最终推荐
...
| **工具** | **MCP Server** | **功能说明** |
| --- | --- | --- |
| 豆瓣 API | douban_mcp.py | 获取影片元数据（类型、时长、评分、简介） |
| IMDb API | imdb_mcp.py | 获取国际影片信息、多语言标题 |
| 日历 API | calendar_mcp.py | 过滤时长约束（如"今晚只有 2 小时"） |
...
│ │ ├── preference_agent.py # Preference Agent

│ │ └── mediator_agent.py # Mediator Agent

│ ├── graph/ # LangGraph 状态机

│ │ └── consensus_graph.py # 协商流程图定义
...
│ │ ├── douban_mcp.py # 豆瓣 API MCP Server

│ │ ├── imdb_mcp.py # IMDb API MCP Server

│ │ └── calendar_mcp.py # 日历 API MCP Server
